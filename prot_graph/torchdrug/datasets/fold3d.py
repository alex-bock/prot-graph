...
class Fold3D(data.ProteinDataset):
    ...
    def __init__(self, path, test_split="test_fold", verbose=1, **kwargs):
        ...
        if test_split not in self.splits[-3:]:
            raise ValueError(
                "Unknown test split `%s` for Fold3D dataset" % test_split
            )
    ...
    def load_hdf5(self, hdf5_file):
        ...
        atom_name = torch.as_tensor(
            [
                data.Protein.atom_name2id.get(name.decode(), -1)
        ...
        for i in range(num_atom):
            if atom2residue[i] != lst_residue:
                residue_type.append(data.Protein.residue2id.get(
                    residue_type_name[i].decode(), 0)
                )
                residue_feature.append(
                    data.feature.onehot(
                        residue_type_name[i].decode(),
                        data.feature.residue_vocab, allow_unknown=True
                    )
                )
        ...
        protein = data.Protein(
            edge_list, atom_type, bond_type, num_node=num_atom,
            num_residue=num_residue, node_position=node_position,
            atom_name=atom_name, atom2residue=atom2residue,
            residue_feature=residue_feature, residue_type=residue_type
        )
    ...
    def load_hdf5s(self, hdf5_files, transform=None, lazy=False, verbose=0):
        ...
        if num_sample > 0:
            warnings.warn(
                """
                Preprocessing proteins of a large dataset consumes a lot of CPU
                memory and time. Use load_pdbs(lazy=True) to construct molecules
    ...
    def get_item(self, index):
        ...
        item = {
            "graph": protein, "fold_label": self.targets["fold_label"][index]
        }
        if self.transform:
